    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-base font-bold text-slate-900 dark:text-slate-100">IssueHub</span>
        </div>
        <HeaderActions />
      </header>
      ...
      <main className="max-w-3xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Workspaces</h2>
          {!creating && (
            <button
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl h-20 animate-pulse" />
            ))}
          </div>
        ) : workspaces.length === 0 ? (
            ...
            {workspaces.map((ws: Workspace) => {
              ...
              return (
                <Link
                  key={ws.id}
                  to={`/workspaces/${ws.id}`}
                  className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 hover:border-violet-300 dark:hover:border-violet-700 hover:shadow-md transition-all flex items-center gap-4"
                >
