...
export const tasksApi = {
  ...
  bulkMove: (taskIds: string[], listId: string) =>
  ...
  listForProject: (projectId: string, params: { page: number; page_size: number; list_id?: string; priority?: Priority; priority_not?: string; assignee_id?: string; tag_ids?: string; status_name?: string; status_name_not?: string; include_subtasks?: boolean; sort_by?: string; sort_dir?: string }) => {
    const p: Record<string, string> = { page: String(params.page), page_size: String(params.page_size) }
    ...
    if (params.assignee_id) p.assignee_id = params.assignee_id
    if (params.tag_ids) p.tag_ids = params.tag_ids
    if (params.status_name) p.status_name = params.status_name
    if (params.status_name_not) p.status_name_not = params.status_name_not
    if (params.include_subtasks) p.include_subtasks = 'true'
    if (params.sort_by) p.sort_by = params.sort_by
    if (params.sort_dir) p.sort_dir = params.sort_dir
